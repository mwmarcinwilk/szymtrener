...
- `templates/mail/reply.html` + `mail/parts.html` (fragmenty `header`, `contact`) — odpowiedź z panelu (`MessageService.sendEmail`) wychodzi w tej samej formatce co potwierdzenie; część tekstowa to dokładnie treść z panelu. Treść idzie przez `th:text`, akapity dzieli `MessageService.paragraphs`. Pilnuje `ReplyMailTest` (w tym escapowanie).
- `Submission.firstName()` — imię do powitania: pierwszy wyraz pola, od wielkiej litery. Używają go potwierdzenie zgłoszenia i szablony odpowiedzi.
- `MessageService.fillBody` — podstawienia w szablonach odpowiedzi. Pusty `{kontekst}` wycina zdanie, w którym stoi (i linię, gdy nic w niej nie zostaje), brak imienia daje „Cześć!". Treści seedów poprawia `V10` tylko tam, gdzie trener ich nie zmienił (`ReplyTemplateMigrationIT`).
...
- Cena Ścieżki 1 (konsultacja + plan) siedzi w `SettingsService.OFFER_CONSULT_PRICE_GR` — jedna kwota, bez własnej tabeli. Tak samo jadłospis dietetyczny: `OFFER_DIET_PRICE_GR` + `OFFER_DIET_VISIBLE`, obsługiwany przez `OnlineOfferService.diet()`. Wyłączenie widoczności chowa kartę na stronie i wycina wzmiankę o jadłospisie ze zdania o cenach w FAQ i w JSON-LD — po zmianie na stronie nie zostaje żadna zaszyta kwota.
...
## CRM: rozmowa, etapy, klienci (handoff §8a i §8b)
- `MessageService` — crm/ — wątek rozmowy. `sendEmail()` wysyła przez JavaMail **synchronicznie** (inaczej niż powiadomienia o zgłoszeniach): trener stoi przy ekranie i musi od razu wiedzieć, czy poszło. Nieudana wysyłka zostaje w wątku jako `FAILED` plus czerwona linia systemowa, zamiast zniknąć w logu. `logPhoneCall()` zapisuje ustalenia i **nic nie wysyła**. `fill(code, imie, kontekst)` podstawia dane w szablonie po stronie serwera.
- `Message` — jedna tabela na cały wątek: zgłoszenie z formularza (`FORM`), pocztę w obie strony (`EMAIL`), notatki z rozmów (`PHONE`) i zdarzenia systemowe (`SYSTEM`). Wątek wyświetla je razem po dacie, więc trzy osobne tabele trzeba by scalać przy każdym otwarciu ekranu.
- `ReplyTemplate` + `/admin/ustawienia` — pięć szablonów odpowiedzi w bazie, nie w JavaScripcie. `code` jest niezmienny: odwołuje się do niego przycisk w rozmowie.
- Etapy: `SubmissionService.stage()` zapisuje datę wejścia w etap (`contacted_at`, `call_booked_at`, `converted_at`, `archived_at`) i **nie kasuje jej** przy powrocie do wcześniejszego kroku. Każda zmiana zostawia wpis systemowy w wątku.
- Przypomnienia: `submission.remind_at` + `ReminderScheduler` (codziennie 7:00, JEDNA zbiorcza wiadomość, nie powiadomienie per zdarzenie). Zaległości widać też jako bursztynową plakietkę przy „Zgłoszeniach" (`AdminNav.dueReminders()`).
- `SubmissionNote` — `pinned`, `tags`, `trainee_id`. `warning()` wybiera notatki z tagiem `Zdrowie`/`Ważne` do karty „Na co uważać". Tagi to pola wyboru, nie wolny tekst: literówka wyłączyłaby tę kartę.
- `ClientInsightService` — crm/ — liczby ekranu klientów, wszystkie z bazy. `packageState()` liczy zużycie z SESJI, nie z licznika w kolumnie; **odwołany trening domyślnie nie zużywa wejścia** (`consumes_package`), a kolor paska zależy od tego, ile ZOSTAŁO, nie od procentu. `attendancePct()` pomija zaplanowane. `progress()` porównuje pierwszy pomiar z ostatnim, a kierunek dobrej zmiany bierze z `measurement.lower_is_better` — z danych, nie z nazwy metryki.
- `Plural` — crm/ — polska odmiana liczebników („1 pakiet", „3 pakiety", „12 pakietów"). Bez tego panel pisał „1 pakietów" przy każdym kliencie.
- Ekrany: `/admin/zgloszenia/{id}` (ścieżka etapów, wątek, pole odpowiedzi z dwoma trybami, notatki na pełną szerokość), `/admin/klienci` (ostrzeżenia o pakietach, 4 KPI, filtry, pasek pakietu), `/admin/klienci/{id}` (profil: dziennik, pomiary, rozmowa, sprzedaż pakietu), `/admin/klienci/{id}/edycja` (dane), `/admin/klienci/tydzien`.
- `static/js/admin-crm.js` — wyłącznie wygoda: tryb odpowiedzi, wstawienie szablonu, zaznaczenie tagu. Każda zmiana danych idzie zwykłym formularzem, więc ekrany działają bez JavaScriptu.
